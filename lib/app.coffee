...
app.get(url.refs(),               [load.repo, load.repo2refs],                                                refs.index)
...
app.get(url.headRefCommits(),     [load.repo, load.headRef, load.ref2commit, load.commit2history],            commits.index)
app.put(url.headRefCommits(),     [load.repo, load.headRef, load.ref2commit],                                 commits.create)
app.get(url.tagRefCommits(),      [load.repo, load.tagRef, load.ref2commit, load.commit2history],             commits.index)
app.get(url.remoteRefCommits(),   [load.repo, load.remoteRef, load.ref2commit, load.commit2history],          commits.index)
