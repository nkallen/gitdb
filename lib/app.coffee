...
module.exports = (resolver) ->
  ...
  app.put(url.headRefCommits(),     [load.repos, load.repo, load.headRef, load.ref2commit],                                 commits.create)
  ...
  app.put(url.commits(),            [load.repos, load.repo],                                                                commits.create)
  app.get(url.commit(),             [load.repos, load.repo, load.commit],                                                   commits.show)
  app.get(url.commitTreeEntry(),    [load.repos, load.repo, load.commit, load.commit2tree, load.entry],                     treeEntries.show)
  app.get(url.tree(),               [load.repos, load.repo, load.tree, load.entry],                                         treeEntries.show)
  app.get('/', [load.repos],
